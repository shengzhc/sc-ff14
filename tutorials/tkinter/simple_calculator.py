...
class SimpleCalculator:
    def __init__(self, application):
        ...
        self.total_label_text.set("Total: {}".format(self.total))
        self.total_label = Label(
            self.application, textvariable=self.total_label_text)

        # Input
        vcmd = self.application.register(self.validate)
        self.entry = Entry(self.application, validate="key",
                           validatecommand=(vcmd, '%P'))

        # Button
        self.add_button = Button(
            self.application, text="+", command=lambda: self.update("add"))
        self.subtract_button = Button(
            self.application, text="-", command=lambda: self.update("subtract"))
        self.reset_button = Button(
            self.application, text="Reset", command=lambda: self.update("reset"))
